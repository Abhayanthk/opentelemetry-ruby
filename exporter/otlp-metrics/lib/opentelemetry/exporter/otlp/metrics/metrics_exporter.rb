        ...
        class MetricsExporter < ::OpenTelemetry::SDK::Metrics::Export::MetricReader # rubocop:disable Metrics/ClassLength
          ...
          def default_aggregation(instrument_kind)
            if instrument_kind == :histogram
              case ENV['OTEL_EXPORTER_OTLP_METRICS_DEFAULT_HISTOGRAM_AGGREGATION']
              ...
              when 'explicit_bucket_histogram', nil
                ...
                OpenTelemetry.logger.warn(
                  "OTEL_EXPORTER_OTLP_METRICS_DEFAULT_HISTOGRAM_AGGREGATION: unrecognized value '#{ENV['OTEL_EXPORTER_OTLP_METRICS_DEFAULT_HISTOGRAM_AGGREGATION']}', defaulting to explicit_bucket_histogram."
                )
                super
              end
            else
